function [tested_phis, tested_etas, Xs, trajectory] = ...
        CASwithoutNorm(eta_ub, X0, ref, gradient, d, V, lambda, Problem,  max_iter, c1_coeff)
    ...
    %% tested_phis -- hypervolumes associated with tested_etas
    %% Xs          -- An array of SOLUTIONs that were generated
    %% trajectory  -- A struct for visualization purposes
    ...
    [tested_phis, tested_etas, Xs, trajectory] = performLineSearch(phi0, phi_prime0, eta_ub, X0, ...
            gradient, Problem, ref, c1_coeff, max_iter, MIN_ETA, CHVmode);
...
function [tested_phis, tested_etas, Xs, trajectory] = performLineSearch(phi0, phi_prime0, eta_ub, X0, ...
    ...
    for iter = 1:max_iter
        ...
        [phi_current, X1] = evaluateStepSize(eta, X0, gradient, Problem, ref, CHVmode);
    ...
    % Return best found
    final_eta = eta;
    [final_phi, X1] = evaluateStepSize(final_eta, X0, gradient, Problem, ref, CHVmode);
    Xs = [Xs, X1];
...
function [phi, X1] = evaluateStepSize(eta, X0, gradient, Problem, ref, CHVmode)
    ...
    x1_flat = x0_flat + gradient .* eta .* repmat((Problem.upper-Problem.lower), 1, N);
...
function success = checkCVCondition(X0, X1)
