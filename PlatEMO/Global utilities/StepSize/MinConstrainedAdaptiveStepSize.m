function [tested_phis, tested_etas, Xs, trajectory] = ...
        MinConstrainedAdaptiveStepSize(eta_ub, X0, ref, gradient, d, V, lambda, Problem, NormStruct, max_iter, c1_coeff)
...
function success = checkCVCondition(X0, X1, NormStruct)
    ...
    [min_CV_X0,~,~] = TotalCV(X0.cons);
    [min_CV_X1,~,~] = TotalCV(X1.cons);
    
    % fprintf("CV(X0): % .4f. CV(X1): % .4f\n", avg_CV_X0, avg_CV_X1);

    success = min_CV_X1 < min_CV_X0;
end
