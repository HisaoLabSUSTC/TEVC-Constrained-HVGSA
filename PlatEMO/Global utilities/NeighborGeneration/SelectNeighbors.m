function [NeighborArr, X0_prime, X0_indices] = SelectNeighbors(X0, Archive, mode, k_min, U)
    ...
    %% We allow inclusion of the team itself,
    %% but we constrain the distance to be > 0
    [Unique_decs, Unique_idx] = unique(Archive_decs, 'rows');
    %% Unique_decs = Archive_decs(Unique_idx)

    %% Compute all squared distances at once (vectorized)
    AllDist2 = pdist2(Archive_decs, X0_decs).^2;  % [nUnique x mu]

    %% Sort each column (each team member's distances)
    [AllDist2_sorted, sort_order] = sort(AllDist2, 1);  % sort along rows
    ...
    dplus1_dists = AllDist2_sorted(row_idx, :);  % 1 x mu
    ...
    switch mode
        ...
        otherwise
            error('SelectNeighbors:badMode', 'Unknown mode: %s', mode);
    end
    ...
    NeighborArr = arrayfun(@(i) Archive(out(i, :)), 1:r, 'UniformOutput', false);
end
