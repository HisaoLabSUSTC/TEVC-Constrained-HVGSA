function [v, V, lambda, d] = CGSA_sn( x0_flat, ...
                                hv_x0_flat, ...
                                c_x0_flat, ...
                                xi_flat, ...
                                hv_xi_flat, ...
    ...
    %% preprocess c_x0_flat to remove negative values
    ineq_idx = c_x0_flat < -tol;
    ineq_mask = true(1, size(c_x0_flat, 2));
    ineq_mask(ineq_idx) = false;

    c_x0_flat = c_x0_flat(ineq_mask);
    c_xi_flat = c_xi_flat(:, ineq_mask);

    d=(hv_xi_flat-hv_x0_flat)./(vecnorm(xi_flat-x0_flat,2,2)+eps); %r*1
    V=((xi_flat-x0_flat)./(vecnorm(xi_flat-x0_flat,2,2)+eps))'; %d*r
    M=((c_xi_flat-c_x0_flat)./(vecnorm(xi_flat-x0_flat,2,2)+eps))'; %np*r
    h = c_x0_flat';
    ...
    %% For each column of V, we can identify their nonzero rows.
    %% We can normalize "groups" of directions
    flat_VL = V*lambda;
    VL = Unflatten(flat_VL,U);
    VL = VL./(vecnorm(VL,2,2)+eps);
    ...
    % disp("VL");FormatMatrix("% .5f", V*lambda);
    % disp("v");FormatMatrix("% .5f", v);
end
