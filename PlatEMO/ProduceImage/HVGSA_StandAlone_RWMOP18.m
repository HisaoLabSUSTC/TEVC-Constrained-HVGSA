...
algorithmConfig = struct('eta', 1e-3, 'gradient_method', 'CGSA_n', 'neighbor_mode', 'archive', 'a', 1, 'ref', ref_pf);
...
set(ax1.ZLabel, 'VerticalAlignment', 'middle')
set(ax1.XLabel, 'VerticalAlignment', 'middle')
set(ax1.YLabel, 'VerticalAlignment', 'middle')
ax1.XTick = ax1.XTick(1:2:end);
ax1.XTickLabel = str2double(ax1.XTickLabel(1:2:end));
ax1.YTick = ax1.YTick(1:2:end);
ax1.YTickLabel = str2double(ax1.YTickLabel(1:2:end));
set(ax1, 'YTickLabelRotation', 0)

fig2 = figure(2);
ax2 = fig2.CurrentAxes;
ax2.XTick = ax2.XTick(1:2:end);
ax2.XTickLabel = str2double(ax2.XTickLabel(1:2:end));
ax2.YTick = ax2.YTick(1:2:end);
ax2.YTickLabel = str2double(ax2.YTickLabel(1:2:end));


fig1 = figure(1); ax1 = fig1.CurrentAxes;
title(ax1, 'RCM18 Decision Space');
filename = sprintf('%s_HVGSA_dec_%d.png', problemName, max_iter);
exportgraphics(fig1, fullfile(imgDir, filename), 'Resolution', 500);

fig2 = figure(2); ax2 = fig2.CurrentAxes;
title(ax2, 'RCM18 Objective Space');
filename = sprintf('%s_HVGSA_obj_%d.png', problemName, max_iter);
