classdef Example1NormAlgorithm < ALGORITHM
    methods
        function main(Algorithm,Problem)
            ...
            while Algorithm.NotTerminated(Population)
                ...
                [Norm_X0_objs, Norm_X0_cons, NormStruct] = ...
                        NormReference(X0.objs, X0.cons, struct(), norm_eps);
                Norm_X0 = SOLUTION(X0.decs, Norm_X0_objs, Norm_X0_cons);
    
                [Neighbors, Norm_Neighbors] = GenerateNormedArtificialNeighbors(X0, Problem, NormStruct, "radial");
                
                % return
    
                cell_objs = cellfun(@(c) c.objs, Norm_Neighbors, 'UniformOutput', false);
                concat_objs = vertcat(cell_objs{:}, Norm_X0.objs);
                ...
                cell_flat_decs = cellfun(@(c) Flatten(c.decs), Norm_Neighbors, 'UniformOutput', false);
                flat_Xi = vertcat(cell_flat_decs{:});
                cell_flat_objs = cellfun(@(c) Flatten(c.objs), Norm_Neighbors, 'UniformOutput', false);
                flat_FXi = vertcat(cell_flat_objs{:});
                cell_flat_cons = cellfun(@(c) Flatten(c.cons), Norm_Neighbors, 'UniformOutput', false);
                ...
                [tps, tes, Xs, trajectory] = ConstrainedAdaptiveStepSize(...
                                                    eta, X0, ref, ...
