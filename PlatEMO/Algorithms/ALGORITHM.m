...
%--------------------------------------------------------------------------
            ...
            try
                ...
                if ~strcmp(err.identifier,'PlatEMO:Termination')
                    if isempty(getCurrentTask())
                        fid = fopen(sprintf('debug_error_run%d.txt', rng().Seed), 'w');
                    else
                        fid = fopen(sprintf('debug_error_lab%d.txt', getCurrentTask().ID), 'w');
                    end
                    if fid ~= -1
                        fprintf(fid, 'RNG state:\n');
                        rngState = obj.rngState;
                        fprintf(fid, 'Type: %s, Seed: %d\n', rngState.Type, rngState.Seed);
                        stateStr = evalc('disp(rngState.State)');
                        fprintf(fid, '%s\n', stateStr);
                        fprintf(fid, 'Algorithm parameters:\n');
                        configStr = evalc('disp(obj.parameter{1})');
                        fprintf(fid, '%s\n', configStr);
    ...
    methods
        ...
        %   set, otherwise returns class(obj).
